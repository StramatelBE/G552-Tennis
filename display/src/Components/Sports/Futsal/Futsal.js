...
function Futsal({ gameState: incomingGameState }) {
          ...
          if (timer > 0) {
            return (
              <div className="time-exclusion-sport" style={{ top: `${140 + index * 35}px`, left: '2px', fontSize: '35px', justifyContent: 'left' }} key={index}>
                <div style={{ color: "#ff0000", width: "25px" }}>{gameState?.Home?.Exclusion?.ShirtNumber[index] !== null || gameState?.Home?.Exclusion?.ShirtNumber[index] !== 0 ? (`${gameState?.Home?.Exclusion?.ShirtNumber[index]}`) : (" ")}
          ...
          if (timer > 0) {
            return (
              <div className="time-exclusion-sport" style={{ top: `${140 + index * 35}px`, right: '2px', fontSize: '35px', justifyContent: 'right' }} key={index}>
                <div style={{ color: "#ff0000", width: "25px" }}>{gameState?.Guest?.Exclusion?.ShirtNumber[index] !== null || gameState?.Guest?.Exclusion?.ShirtNumber[index] !== 0 ? (`${gameState?.Guest?.Exclusion?.ShirtNumber[index]}`) : (" ")}
                </div>
                <span>&nbsp;</span>
                <div style={{ color: "#ebff00" }}>{formatExclusionTimer(timer) || ""}
                </div>
